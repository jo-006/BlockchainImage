...
def new_transaction():
    ...
    required_fields = ["user", "v_file", "file_data", "file_size", "image_hash"]
    ...
    # Verify if the image has been uploaded before and if it's been tampered with
    filename = file_data.get("v_file")
    image_hash = file_data.get("image_hash")

    # Check if we have a record of this image
    verification = blockchain.verify_image(filename, image_hash)
    if verification:
        if not verification['verified']:
            # Image has been tampered with
            return jsonify({
                'verified': False,
                'message': 'REJECTED: Image has been tampered with!',
                'original_hash': verification['original_hash'],
                'uploaded_hash': verification['uploaded_hash']
            }), 400

    # append it to pending transaction
...
@app.route("/image_hashes", methods=["GET"])
# Get all registered image hashes
def get_image_hashes():
    return jsonify(blockchain.image_hashes)
