...
class Blockchain:
    ...
    def __init__(self):
        ...
        self.image_hashes = {}  # Dictionary to store image hashes
    ...
    def add_block(self, block, hashl):
        ...
        if (prev_hash == block.prev_hash and self.is_valid(block, hashl)):
            ...
            # Update image hashes with any new transactions
            for tx in block.transactions:
                if 'v_file' in tx and 'image_hash' in tx:
                    self.image_hashes[tx['v_file']] = tx['image_hash']
            return True
        else:
            return False
    ...
    # Verify image hash against stored hashes
    def verify_image(self, filename, image_hash):
        # If the image has been registered before
        if filename in self.image_hashes:
            original_hash = self.image_hashes[filename]
            # Check if the hashes match
            return {
                'verified': image_hash == original_hash,
                'original_hash': original_hash,
                'uploaded_hash': image_hash
            }
        # If image hasn't been registered yet
        return None